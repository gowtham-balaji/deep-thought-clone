...
for (let i = 0; i < project.tasks.length; i++) {
  ...
  for (let j = 0; j < task.assets.length; j++) {
    const asset = task.assets[j];

    const assetElement = document.createElement("li");
    assetElement.innerHTML = `
      <h3 class = "title">${asset.asset_title}</h3> <span class="collapse"><button>i</button></span>
      <p class = "desc"> <span class="line">Description</span> :${asset.asset_description}</p>
      <iframe class = "video" src="${asset.asset_content}" width = "100%" height = "400px"></iframe>
     
    `;

    const descElement = assetElement.querySelector(".desc");
    descElement.classList.remove("desc-content-hidden");

    const toggleButton = assetElement.querySelector(".collapse");
    toggleButton.addEventListener("click", () => {
      descElement.classList.toggle("desc-content-hidden");
    });
